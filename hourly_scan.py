...
def run_hourly(offline=False):
    ...
    # TELEGRAM:
    # - Never send the normal NEW SIGNAL message here; news-followup sends
    #   the richer one-per-ticker report.
    # - ALWAYS send position/thesis alerts. These are state-change alerts,
    #   not duplicate signal notifications.
    for m in pos_msgs:
        notify.send_telegram(m, cfg)

    # If there are no NEW signals, send the hourly no-signal status to both
    # email and Telegram.
    if not signals:
